...
- 针对发送端发出的数据包的**确认应答**信号ACK
- 针对数据包丢失或者出现定时器**超时的重传**机制
- 针对数据包到达接收端主机顺序乱掉的**顺序控制**
- 针对高效传输数据包的**滑动窗口**控制
- 针对避免网络拥堵时候的**拥塞控制**
- 针对刚开始启动的时候避免一下子发送大量数据包而导致网络瘫痪的慢启动算法和**拥塞控制**。
...
在某段时间，若对网络中某一资源的需求超过了该资源所能提供的可用部分，网络的性能就要变换，叫做拥塞。拥塞窗口的单位是mss（一个报文段的大小）。
...
1.当发送方连续收到三个重复确认，执行乘法减小，ssthresh减半。（连续收到的三个ack都是由于收到了不对的包而发出的对这些不对的包的ack，并不是说接收方有主动发ack的能力，当连续触发三次以后，就提前超时重传）。
